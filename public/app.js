...
function connect() {
    ...
    if (uniqueId !== '') {
        ...
        $('#stateText').text('Əlaqə Qurulur...');

        connection.connect(uniqueId, {
            enableExtendedGiftInfo: true
        }).then(state => {
            $('#stateText').text(`Otaq Nömrəsi ${state.roomId}`);
...
function updateRoomStats() {
    $('#roomStats').html(`İzləyici Sayı: <b>${viewerCount.toLocaleString()}</b> Bəyəni: <b>${likeCount.toLocaleString()}</b> Hədiyyə Jeton Sayı: <b>${diamondsCount.toLocaleString()}</b>`)
}
